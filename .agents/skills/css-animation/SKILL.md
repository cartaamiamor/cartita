...
> **Origen:** descargada de [`neonwatty/css-animation-skill`](https://github.com/neonwatty/css-animation-skill/blob/main/SKILL.md) el 2026-08-20. Licencia: MIT (confirmada en `LICENSE` del repo, © Jeremy Watt). Contenido sin modificar respecto al original.
>
> **Nota para este proyecto:** esta skill fue pensada para generar demos/onboarding de una app ya construida, apoyándose en Claude-in-Chrome y un servidor Python local (`python3 -m http.server`) para el ciclo de revisión (Fase 4). En este sitio estático sin backend puede seguir siendo útil como generador de un HTML/CSS autocontenido (p. ej. una animación de "carta que se abre" para incrustar en `index.html`), pero probablemente convenga usar solo las Fases 3–4 (Generar + Revisar) o adaptarlas manualmente, saltando la Fase 1 de investigación de una app en vivo.
